...
- `working_group_reports`: `id`, `campaign_id → reporting_campaigns`,
  `working_group_id → organisational_units`, `status` (`draft` | `submitted` | `accepted`), unique
  on `(campaign_id, working_group_id)`
...
The admin user create/edit forms now have an "Actor link" section with a role selector (`none` |
`person` | `country`) and a searchable picker for the chosen type. The pickers reuse the existing
`ContributionOptionPicker` component. A `countries` resource was added to the
...
- **Regular users** — edit and/or confirm only the specific reports they are responsible for. Pages
  live under a new `/dashboard/reporting/` section (not under `/administrator/`).
...
2. If `user.personId` is set, query the person's active `personsToOrganisationalUnits` relations to
   ...
   `working_group_reports` and `country_reports` for the current open campaign to get report IDs and
   statuses.
