-- Derive the country statistics from the members_and_partners view instead of a parallel
-- hand-rolled join tree, so there is a single SQL definition of DARIAH membership (the view is
-- pinned to the dariah-eu eric since 20260712120000). Also expose the observer-countries count.
-- Rows in members_and_partners are unique per (published country version, status); COUNT(DISTINCT
-- "id") stays exact even if a future view change were to produce duplicates.
