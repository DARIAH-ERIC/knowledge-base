...
-- Guard on the legacy column so this is a no-op when rerun or applied after `db:push` has already
-- created the current schema.
DO $$
BEGIN
	IF EXISTS (
		SELECT 1
		...
		WHERE table_schema = 'public'
			AND table_name = 'working_group_reports'
			AND column_name = 'mailing_list'
	) THEN
