...
function Ring(props: Readonly<SVGProps<SVGSVGElement>>): ReactNode {
...
function Spin(props: Readonly<SVGProps<SVGSVGElement>>): ReactNode {
	const { className, ...rest } = props;
...
export interface LoaderProps extends Omit<ComponentProps<"svg">, "display" | "opacity" | "intent"> {
...
export function Loader(props: Readonly<LoaderProps>): ReactNode {
	const { className, isIndeterminate = true, ref, variant = DEFAULT_SPINNER, ...rest } = props;

	const t = useExtracted("ui");

	const LoaderComponent = LOADERS[variant in LOADERS ? variant : DEFAULT_SPINNER];

	return (
		<ProgressBar
			// eslint-disable-next-line @eslint-react/prefer-destructuring-assignment
			aria-label={props["aria-label"] ?? t("Pending...")}
			data-slot="loader"
			// eslint-disable-next-line @eslint-react/prefer-destructuring-assignment
