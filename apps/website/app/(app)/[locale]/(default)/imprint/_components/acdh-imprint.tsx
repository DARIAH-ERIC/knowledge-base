...
async function getImprintHtml(locale: IntlLocale): Promise<string> {
	const url = createUrl({
		baseUrl: env.NEXT_PUBLIC_WEBSITE_IMPRINT_SERVICE_BASE_URL,
		pathname: `/${String(env.NEXT_PUBLIC_WEBSITE_SERVICE_ID)}`,
		searchParams: createUrlSearchParams({
			locale,
			redmine: env.NEXT_PUBLIC_WEBSITE_IMPRINT_CUSTOM_CONFIG,
		}),
	});
...
export async function AcdhImprint(props: Readonly<AcdhImprintProps>): Promise<ReactNode> {
	...
	return (
		...
		<div className="richtext max-inline-(--breakpoint-md)" dangerouslySetInnerHTML={{ __html: html }} />
	);
}
