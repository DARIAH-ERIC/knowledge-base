...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
	...
	for await (const path of glob("./**/page.tsx", {
		...
		exclude(fileName) {
			return fileName.startsWith("(auth)") || fileName.startsWith("(dashboard)");
		},
	})) {
	...
	const entries = locales.flatMap((locale) => routes.map((pathname) => {
			return {
				url: String(createUrl({ baseUrl, pathname: getPathname({ href: { pathname }, locale }) })),
				/**
				 * Only add `lastmod` when the publication date is actually known. Don't use the build date
				 * instead.
				 */
