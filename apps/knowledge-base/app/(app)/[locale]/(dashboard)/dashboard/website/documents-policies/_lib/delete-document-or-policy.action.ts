...
export async function deleteDocumentOrPolicyAction(id: string): Promise<void> {
	await assertAdmin();
	...
	await db.transaction(async (tx) => {
		...
		await tx.delete(schema.documentsPolicies).where(eq(schema.documentsPolicies.id, id));
