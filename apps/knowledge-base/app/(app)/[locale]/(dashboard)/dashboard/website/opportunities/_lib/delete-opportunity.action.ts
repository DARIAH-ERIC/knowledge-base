...
export const deleteOpportunityAction = createCommandAction({
	requireAdmin: true,
	audit: { action: "delete", subjectType: "opportunities" },
	...
	async mutate(tx, [documentId]: [string]) {
		...
		for (const versionId of versionIds) {
			await opportunitiesLifecycleAdapter.wipeSubtype(tx, versionId);
		}
