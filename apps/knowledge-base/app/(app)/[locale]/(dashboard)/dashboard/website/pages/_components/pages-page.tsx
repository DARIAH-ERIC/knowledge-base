...
export function PagesPage(props: Readonly<PagesPageProps>): ReactNode {
	...
	return (
		<Fragment>
			<EntityListHeader
				title={t("Pages")}
				description={t("Manage all pages in the DARIAH knowledge base.")}
				action={
					<>
						<EntityListSearchField search={search} />
						<NewLink href="/dashboard/website/pages/create">{t("New")}</NewLink>
					</>
				}
			/>
					...
					{(item) => (
						<TableRow href={`/dashboard/website/pages/${item.entity.slug}/details`}>
							...
							<TableCell className="sticky inset-e-0 z-10 bg-linear-to-l from-60% from-bg text-end">
								<RowActionsMenu>
									<RowActionsMenu.Link
										href={`/dashboard/website/pages/${item.entity.slug}/details`}
										icon={<EyeIcon className="me-2 block-4 inline-4" />}
									>
										{t("View")}
									</RowActionsMenu.Link>
									<RowActionsMenu.Link
										href={`/dashboard/website/pages/${item.entity.slug}/edit`}
			...
			<EntityListPagination search={search} total={pages.total} pageSize={pageSize} />
			...
			<EntityDeleteModal
				...
				onConfirm={() => {
					...
					startDeleteTransition(async () => {
						...
						try {
							const state = await deletePageItemAction(documentId);
							if (isActionStateError(state)) {
								const message = Array.isArray(state.message) ? state.message[0] : state.message;
								setDeleteError(message ?? t("Could not delete page. Please try again."));
								return;
							}
							router.refresh();
							setItemToDelete(null);
						} catch {
							setDeleteError(t("Could not delete page. Please try again."));
						}
					});
				}}
			/>
