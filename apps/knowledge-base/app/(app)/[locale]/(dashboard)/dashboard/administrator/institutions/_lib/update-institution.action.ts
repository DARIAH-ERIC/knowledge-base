...
export const updateInstitutionAction = createServerAction(
	async function updateInstitutionAction(state, formData) {
		...
		const result = await v.safeParseAsync(
			UpdateInstitutionActionInputSchema,
			getFormDataValues(formData),
			{ lang: getIntlLanguage(locale) },
		);

		if (!result.success) {
			const errors = v.flatten<typeof UpdateInstitutionActionInputSchema>(result.issues);
