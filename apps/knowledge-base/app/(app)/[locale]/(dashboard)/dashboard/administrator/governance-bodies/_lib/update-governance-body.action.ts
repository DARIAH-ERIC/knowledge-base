...
export const updateGovernanceBodyAction = createServerAction(
	async function updateGovernanceBodyAction(state, formData) {
		...
		const result = await v.safeParseAsync(
			UpdateGovernanceBodyActionInputSchema,
			getFormDataValues(formData),
			{ lang: getIntlLanguage(locale) },
		);

		if (!result.success) {
			const errors = v.flatten<typeof UpdateGovernanceBodyActionInputSchema>(result.issues);
