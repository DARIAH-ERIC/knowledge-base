...
export async function verifyPasswordResetEmailAction(
	...
	const { session } = await auth.validatePasswordResetSessionRequest();
	...
	const emailMatches = await auth.setUserAsEmailVerifiedIfEmailMatches(session.userId, session.email);
	if (!emailMatches) {
