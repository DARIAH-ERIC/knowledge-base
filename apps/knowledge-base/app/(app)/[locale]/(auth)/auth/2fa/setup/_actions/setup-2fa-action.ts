...
import { decodeBase64, verifyTOTP } from "@dariah-eric/auth";
import { getLocale, getTranslations } from "next-intl/server";
import * as v from "valibot";

import { urls } from "@/config/auth.config";
import { getCurrentSession, setSessionAs2FAVerified, updateUserTOTPKey } from "@/lib/data/users";
...
export async function setup2faAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
	...
	const t = await getTranslations("actions.setup2FAAction");
