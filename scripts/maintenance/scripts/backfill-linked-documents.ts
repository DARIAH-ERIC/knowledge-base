...
async function findTargets(): Promise<Array<Target>> {
	...
	return [
		...richText.map((row): Target => ({ table: "rich_text", ...row })),
		...callout.map((row): Target => ({ table: "callout", ...row })),
		...mediaText.map((row): Target => ({ table: "media_text", ...row })),
		...accordion.map((row): Target => ({ table: "accordion", ...row })),
	];
}
